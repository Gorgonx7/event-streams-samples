 ...
 * Uses an AVRO schema to validate the data sent and read.
 *
 * @author IBM
 */
public class EventStreamsConsoleSample {
    ...
    private static void printUsage() {
        System.out.println("\n"
                ...
                + "              <kafka_brokers_sasl> <kafka_http_url> { <api_key> | <user = token>:<password> } [" + ARG_CONSUMER + "] \\\n"
                + "               [" + ARG_TOPIC + "]\n"
                + "Where:\n"
                + "    kafka_broker_sasl\n"
                ...
                + "    kafka_http_url\n"
                + "        Required. Endpoint to be used for schema registry admin operations, for\n"
                + "        example \"https://mh-example-0000.us-south.containers.appdomain.cloud\".\n"                
    ...
    public static void main(String args[])  {
        try {
            ...
            if (args.length == 0) {
                ...
                registryUrl = credentials.getKafkaHttpUrl();
            } else {
                ...
                if (args.length > 3) {
                    try {
                        ...
                        if (!parsedArgs.containsKey(ARG_CONSUMER)) {
                            runConsumer = false;
                        }
            ...
            logger.info("Schema Registry Endpoint: {}", registryUrl);

            final String registrySerDesUrl = registryUrl + "/confluent";
            final String registrySerDesUrlWithAuth = registrySerDesUrl.replace("https://","https://token:" + apiKey + "@");
            ...
            if (runConsumer) {
                Map<String, Object> consumerConfigs = getConsumerConfigs(bootstrapServers, apiKey, registrySerDesUrlWithAuth);
    ...
    static final Map<String, Object> getConsumerConfigs(String bootstrapServers, String apikey, String registryURL) {
        Map<String, Object> configs = new HashMap<>();
        configs.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class.getName());
        configs.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, KafkaAvroDeserializer.class.getName());
        configs.put(KafkaAvroDeserializerConfig.SCHEMA_REGISTRY_URL_CONFIG, registryURL);
        configs.put(KafkaAvroDeserializerConfig.BASIC_AUTH_CREDENTIALS_SOURCE, "URL");
